...
function summa(uri, topK, language, fixedProperty, id, service) {
	...
	$.ajax({
		...
		success : function(data) {
			...
			//add to html
			$("#" + id).append("<div style='float:right' id='" + id + "_close'>x</div><h2>" + label(print.entity) + "</h2>" + "<table class='abstract'></table><table class='points'></table>");
			...
			$("#" + id).append("<i style='font-size:10px'>_______<br>Summary by: <a href='" + service.substring(0, service.lastIndexOf("/")) + "'>" + service.substring(0, service.lastIndexOf("/")) + "</a>" + "<br>Sources: <a href='http://dbpedia.org'>http://dbpedia.org</a>" + ", <a href='http://wikipedia.org'>http://wikipedia.org</a>" + ", <a href='http://duckduckgo.com/'>http://duckduckgo.com/</a></i>");
			$("#" + id).show();
			...
			$.ajax({
				...
				success : function(data2) {
					...
					//get wiki-link
					var identifier = uri.split("/");
					var wikiurl = "https://" + language + ".wikipedia.org/wiki/" + identifier[identifier.length - 1];
					...
					if (statement2["text"].length < 100) {
						...
						var tabletext = "<span class='short' style='background-color: transparent;'>" + shorttext + "</span>" + "<span class='rest' style='background-color: transparent;'>" + resttext + "<a target='blank' href=" + wikiurl + ">(read more on <IMG style='height:1em;' SRC='css/images/wiki.png'>)</a>" + "</span>" + " " + "<a href='#' class='more'>" + "(more...)" + "</a>";
